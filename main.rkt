...
(define (crack-cipher strategies key)
  (define allkeys
    (algo:remove-duplicates
     (append*
      (map (lambda(y) (map (lambda(x) (utils:add-substitution x empty-key)) (y empty-key))) strategies))));; Returns list of encryption keys.
  (let ([list-of-keys (algo:remove-duplicates (cracking-with-all-keys allkeys 0))])
    (begin
      (map (lambda(x) (utils:show-key x)) list-of-keys)
      list-of-keys)))
(define (cracking-with-all-keys allkeys index)
  (cond
   [(null? allkeys) allkeys]
   [else      
    (let
        ([after-d-c (algo:dictionary-closure (car allkeys))])
      (cond
        [(equal? #f after-d-c)
         (cracking-with-all-keys (cdr allkeys) index)]
          [else
           (let([after-swe (algo:secret-word-enumeration after-d-c)])
             (cond
               [(equal? #f after-swe)
                (cracking-with-all-keys (cdr allkeys) index)]
               [else
                (begin (displayln (string-append "@@@+ " (~a index)))
                              (utils:show-key after-swe)
                              (displayln "@@@-")
                              (displayln "...")
                              (displayln "...")
                              (append (list after-swe) (cracking-with-all-keys (cdr allkeys) (+ index 1))))]))]))
    ]))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;                                 Optional task                                    ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (crack-hard-cipher strategies key) ;; Returns a list of encryption keys.
  ...
  ;; DISPLAY A KEY AS SOON AS YOU FIND IT USING (show-key key)
  (list key))
(define empty-key strat:emptykey)